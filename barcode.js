export function BARCode(options) {
	...
	let {message, width = 320, height = 80, horizontalPadding = 20, verticalPadding = 16, foreground = '#000', background} = options;
	let w = abs(width),
		h = abs(height),
	...
	return (function() {
		...
		let r, ns = 'http://www.w3.org/2000/svg', path = '',
			c = l, d = 0;
		while (c) {
			message[--c] && ++d && !message[c - 1] && (
			path += (dir)
			? 'M1,' + c + 'H0v' + d + 'h1v-' + d + 'z' : 'M' + c + ',1h' + d + 'V0h-' + d + 'v1z',
			d = 0);
		}
